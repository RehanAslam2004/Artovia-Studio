...
/**
 * Helper to get all admin emails (combines defaults with env variables)
 */
function getAdminEmails() {
    const defaultEmails = ['artovia.business@gmail.com', 'ayeshakhann.1519@gmail.com'];
    const envEmails = process.env.NEXT_PUBLIC_ADMIN_EMAILS
        ? process.env.NEXT_PUBLIC_ADMIN_EMAILS.split(',').map(e => e.trim().toLowerCase())
        : [];
    return Array.from(new Set([...defaultEmails, ...envEmails])).filter(Boolean);
}
...
export async function sendAdminOrderNotification(order) {
    ...
    return { success, error: success ? null : results.find(r => r.error)?.error };
}
...
export async function sendCustomQuoteEmail(requestData, price) {
    ...
    const body = `
        ...
        <div style="background-color: #f9fafb; padding: 20px; border-radius: 12px; border: 1px solid #e5e7eb; margin: 20px 0; display: flex; flex-direction: column; gap: 15px;">
            <div>
                <strong style="color: #4b5563;">EasyPaisa</strong><br>
    ...
    try {
        ...
        const zipBuffer = await new Promise(async (resolve, reject) => {
            ...
            for (const file of presetFiles) {
                try {
                    // Sanitize the filename to prevent ZIP extraction errors (especially on Windows due to | : * ? " < > etc.)
                    ...
                    // Normalize the URL
                    let fileUrl = file.url.trim();
                    if (fileUrl.startsWith('//')) {
                        fileUrl = 'https:' + fileUrl;
                    }

                    console.log(`[Preset Email] Downloading: ${cleanName} from ${fileUrl}`);
                    const response = await fetch(fileUrl);
                    if (!response.ok) {
                        console.error(`[Preset Email] Failed to fetch ${cleanName}: ${response.statusText}`);
                        continue;
                    }
                    const fileBuffer = Buffer.from(await response.arrayBuffer());
                    archive.append(fileBuffer, { name: cleanName });
                    console.log(`[Preset Email] Added ${cleanName} (${(fileBuffer.length / 1024 / 1024).toFixed(2)} MB)`);
                } catch (fileErr) {
...
/**
 * Send bookmark delivery email with PDF file attached directly
 * @param {Object} order - Order data
 * @param {Array} pdfFiles - Array of { name, url } objects for PDF files
 */
export async function sendBookmarkDeliveryEmail(order, pdfFiles = []) {
    const { userEmail, userName, id: orderId } = order;
    const siteUrl = getSiteUrl();
    const downloadPageUrl = `${siteUrl}/download/${orderId}`;

    // Build file list HTML
    const fileListHtml = pdfFiles.map(f =>
        `<div style="display: flex; align-items: center; gap: 8px; padding: 8px 0; border-bottom: 1px solid #f3f4f6;">
            <span style="color: #ec4899; font-size: 18px;">🔖</span>
            <span style="font-weight: 500; color: #374151;">${f.name}</span>
        </div>`
    ...
    const body = `
        ...
        <h2 style="text-align: center;">Your Bookmark is Ready! 🔖</h2>
        <p>Great news, <strong>${userName}</strong>! Your payment has been verified and your printable bookmark PDF is attached to this email.</p>
        ...
        <div style="background-color: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #e5e7eb;">
            ...
            <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #e5e7eb;">
                <p style="margin: 0; font-size: 13px; color: #6b7280;">
                    <strong>📎 Check the attachment below</strong> — Your bookmark PDF is ready to print at home or at a print shop.
                </p>
            </div>
        </div>

        <div style="margin-top: 25px; background-color: #fdf2f8; border: 1px solid #fbcfe8; padding: 15px; border-radius: 8px;">
            <h3 style="margin: 0 0 10px 0; font-size: 14px; color: #be185d;">💡 Printing Tips</h3>
            <ol style="margin: 0; padding-left: 20px; font-size: 13px; color: #ec4899;">
                <li>Download the attached PDF file</li>
                <li>Print on card stock or thick paper for best results</li>
                <li>Cut along the edges and your bookmark is ready!</li>
                <li>Laminating adds durability and a premium finish</li>
            </ol>
        </div>
    ...
    // Attach each PDF directly (no ZIP — PDFs are small and there's only one per product)
    let attachments = [];
    try {
        console.log(`[Bookmark Email] Attaching ${pdfFiles.length} PDF file(s)...`);
        for (const file of pdfFiles) {
            try {
                const cleanName = (file.name || 'bookmark.pdf')
                    .replace(/[\\/:*?"<>|]/g, '-')
                    .replace(/\s+/g, ' ')
                    .trim();
                ...
                // 1. raw/upload (if it was correctly uploaded as raw type — new uploads after our fix)
                // 2. image/upload with fl_attachment flag (forces original file download for image-type resources)
                // 3. Original URL as-is (last resort)
                const urlsToTry = [];

                if (fileUrl.includes('cloudinary.com')) {
                    if (fileUrl.includes('/raw/upload/')) {
                        // Already raw — try as-is first, then image fallback
                        urlsToTry.push(fileUrl);
                        urlsToTry.push(fileUrl.replace('/raw/upload/', '/image/upload/fl_attachment/'));
                    } else if (fileUrl.includes('/image/upload/')) {
                        // Stored as image — add fl_attachment to force raw PDF download
                        const withAttachment = fileUrl.replace('/image/upload/', '/image/upload/fl_attachment/');
                        urlsToTry.push(withAttachment);
                        // Also try raw/upload (in case it was somehow dual-stored)
                        urlsToTry.push(fileUrl.replace('/image/upload/', '/raw/upload/'));
                        urlsToTry.push(fileUrl); // original as last resort
                    } else {
                        urlsToTry.push(fileUrl);
                    }
                } else {
                ...
                for (const url of urlsToTry) {
                    try {
                        console.log(`[Bookmark Email] Trying URL: ${url}`);
                        const response = await fetch(url, { signal: AbortSignal.timeout(20000) });
                        if (response.ok) {
                            fileBuffer = Buffer.from(await response.arrayBuffer());
                            console.log(`[Bookmark Email] Success! Downloaded ${cleanName} (${(fileBuffer.length / 1024 / 1024).toFixed(2)} MB) from ${url}`);
                            break;
                        }
                        console.warn(`[Bookmark Email] URL failed (${response.status}): ${url}`);
                    } catch (fetchErr) {
                        console.warn(`[Bookmark Email] Fetch error for ${url}:`, fetchErr.message);
                    }
                }
                ...
                if (fileBuffer) {
                    ...
                    console.error(`[Bookmark Email] All URLs failed for ${cleanName}. Email will send without PDF attachment.`);
                    console.error(`[Bookmark Email] HELP/FIX: This is almost certainly because PDF delivery is blocked by default in your Cloudinary account security settings.`);
                    console.error(`[Bookmark Email] TO FIX: Log in to the Cloudinary Console -> Settings -> Security -> Scroll down to "PDF and ZIP files delivery" -> Check "Allow delivery of PDF and ZIP files" -> Save.`);
                }
            } catch (fileErr) {
                console.error(`[Bookmark Email] Error processing ${file.name}:`, fileErr);
            }
        }
    } catch (attachError) {
        console.error('[Bookmark Email] Failed to process PDF(s):', attachError);
        // Proceed without attachment — email still sends with order page link
    }

    return sendEmailWithAttachments({
        to: userEmail,
        subject: `Your Bookmark PDF is Ready! 🔖 - #${orderId.slice(0, 8).toUpperCase()}`,
        html: generateBaseTemplate({
            title: 'Bookmark Delivered',
            previewText: 'Your printable bookmark PDF is attached and ready to print!',
            body
        }),
        attachments,
    });
