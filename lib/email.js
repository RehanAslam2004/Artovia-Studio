    ...
    try {
        ...
        for (const file of pdfFiles) {
            try {
                ...
                // Cloudinary may have stored the PDF as 'image/upload' resource type
                // (auto-detect sometimes treats PDF as image). Force 'raw/upload' so
                // we download the actual PDF binary, not a rendered image preview.
                if (fileUrl.includes('cloudinary.com') && fileUrl.includes('/image/upload/')) {
                    fileUrl = fileUrl.replace('/image/upload/', '/raw/upload/');
                    console.log(`[Bookmark Email] Corrected URL to raw: ${fileUrl}`);
                }

                console.log(`[Bookmark Email] Downloading: ${cleanName} from ${fileUrl}`);
                const response = await fetch(fileUrl);
                if (!response.ok) {
                    console.error(`[Bookmark Email] Failed to fetch ${cleanName}: ${response.statusText}`);
                    continue;
                }
                const fileBuffer = Buffer.from(await response.arrayBuffer());
                attachments.push({
                ...
                console.log(`[Bookmark Email] Attached ${cleanName} (${(fileBuffer.length / 1024 / 1024).toFixed(2)} MB)`);
            } catch (fileErr) {
                console.error(`[Bookmark Email] Error downloading ${file.name}:`, fileErr);
            }
        }
    } catch (attachError) {
        console.error('[Bookmark Email] Failed to attach PDF(s):', attachError);
